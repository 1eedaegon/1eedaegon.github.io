...
test.describe('Wikilink rendering', () => {
  test('[[slug|label]] renders as an internal link in the article body', async ({ page }) => {
    await page.goto('/articles/astro-series-part2')
    const prose = page.locator('.prose')
    const link = prose.locator('a[href*="/articles/astro-series-part1"]').first()
    await expect(link).toBeVisible()
    await expect(link).toHaveText('Part 1')
  })

  test('article body contains no raw [[ wikilink syntax', async ({ page }) => {
    for (const path of ['/articles/astro-series-part2', '/articles/astro-tips']) {
  ...
  test('![[slug]] renders an embed card with the target title', async ({ page }) => {
    await page.goto('/articles/draft-example')
    const embed = page.locator('.wikilink-embed')
    await expect(embed).toBeVisible()
    await expect(embed.locator('.wikilink-embed-title')).toHaveText('Welcome to Monochrome Edge Blog')
    await expect(embed).toHaveAttribute('href', /\/articles\/welcome-to-monochrome-edge\//)
  })

  test('unknown wikilink target renders as a harmless broken-link span', async ({ page }) => {
    await page.goto('/articles/draft-example')
...
test.describe('Code block chrome', () => {
  test('language watermark renders inside the code box (no chip)', async ({ page }) => {
    await page.goto('/articles/draft-example')
    const watermark = page.locator('.code-block-wrapper .code-watermark').first()
    await expect(watermark).toBeVisible()
    await expect(watermark.locator('.code-watermark-name')).toHaveText('TypeScript')
    await expect(watermark.locator('svg.code-watermark-icon')).toHaveCount(1)
    // faint, not a solid badge
    const opacity = await watermark.evaluate((el) => parseFloat(getComputedStyle(el).opacity))
    expect(opacity).toBeGreaterThan(0.2)
    expect(opacity).toBeLessThan(0.9)
  })

  test('copy button is visible inside the box without hovering', async ({ page }) => {
    await page.goto('/articles/draft-example')
...
test.describe('Draft badge', () => {
  test('draft article shows a DRAFT badge next to the title', async ({ page }) => {
    await page.goto('/articles/draft-example')
