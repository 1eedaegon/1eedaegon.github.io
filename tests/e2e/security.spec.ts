...
test.describe('Security and metadata', () => {
  test('article structured data renders in head and external links are isolated', async ({ page }) => {
    ...
    expect(jsonLdScripts.some((script) => script.includes('"BlogPosting"'))).toBe(true)
  ...
  test('search query is rendered as text, not executable markup', async ({ page }) => {
  ...
  test('search index exposes only same-origin article URLs', async ({ request, baseURL }) => {
