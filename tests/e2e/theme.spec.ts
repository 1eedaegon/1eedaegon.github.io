...
test.describe('Theme', () => {
  test('uses cold theme as the default variant', async ({ page }) => {
    ...
    await expect(page.locator('html')).toHaveAttribute('data-theme-variant', 'cold')
    await expect(page.locator('#theme-toggle')).toHaveAttribute('data-state', 'cold')

    const storedTheme = await page.evaluate(() => localStorage.getItem('theme-variant'))
    expect(storedTheme).toBeNull()
  })

  test('legacy blog tokens resolve to Monochrome Edge theme colors', async ({ page }) => {
    ...
    const tokens = await page.evaluate(() => {
      ...
      const resolveColor = (cssVar: string, property: 'backgroundColor' | 'color' | 'borderColor') => {
  ...
  test('theme switches update actual colors, not only attributes', async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('theme-variant', 'warm')
      localStorage.setItem('theme', 'light')
    })
    ...
    const warmLightBackground = await page.locator('body').evaluate((body) =>
    ...
    const coldLightBackground = await page.locator('body').evaluate((body) =>
      getComputedStyle(body).backgroundColor
    )
  ...
  test('theme controls do not bind duplicate handlers after swaps', async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('theme-variant', 'warm')
      localStorage.setItem('theme', 'light')
    })
  ...
  test('theme persists after navigation', async ({ page }) => {
    ...
    const themeAfterNav = await page.locator('html').getAttribute('data-theme-variant')
    expect(themeAfterNav).toBe(themeAfterToggle)

    // Verify localStorage
    const stored = await page.evaluate(() => localStorage.getItem('theme-variant'))
